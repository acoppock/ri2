...
* Added a `potential_outcomes` argument to `conduct_ri()` and a new `add_potential_outcomes()` function (#19). Naming one column of `data` per treatment condition states the sharp hypothesis as a full schedule of potential outcomes, which allows hypothesized effects that vary across units. `sharp_hypothesis` can only express a constant shift per arm. `add_potential_outcomes()` builds such a schedule and accepts a per-unit vector or matrix of effects, so a null like "the effect is `0.5 * X` for each unit" is one line. Supplied columns are checked against the observed outcomes, which catches columns given in the wrong order.
